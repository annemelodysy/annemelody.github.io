...
$(document).ready(function() {
  ...
  $.fn.onscreen = function() {
    ...
    }), $(".volume").each(function(t) {
      var o = $(this).find(".audio").get(0);
      $(this).onscreen(!0) && $(".icon-volume-up").length > 0 ? o.play() : o.pause()
    })
  });
  ...
  $(window).scroll(function(){if($("section").hasClass("transNight")){var o=$(".sunset").offset().top,t=$(".night").offset().top,n=[250,250,250],a=[0,0,0],s=[a[0]-n[0],a[1]-n[1],a[1]-n[0]],r=($(this).scrollTop()-o)/(t-o);r=Math.min(1,Math.max(0,r));var h=[Math.round(n[0]+s[0]*r),Math.round(n[1]+s[1]*r),Math.round(n[2]+s[2]*r)];$(".journal-post").css("background-color","rgb("+h.join(",")+")")}});

  if (bgWidth > 767) {
    var yearlyItem = $('.yearly-bg .item')
    //yearly
    ...
    $('body').mouseup(function(e){
      if (!yearlyItem.is(e.target) && yearlyItem.has(e.target).length === 0 && !$(e.target).is('.yearly-bg .item'))
      {
        yearlyItem.removeClass('open');
        yearlyItem.find('p').hide(200);
      }
    });
  }
});
